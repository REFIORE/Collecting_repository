...
## Описание
Скрипт ищет фильм по его оригинальному названию в загруженной базе данных и рекомендует до 8 похожих фильмов, исключая сам искомый фильм из результатов.

## Что делает скрипт?

Скрипт находит фильмы, похожие на заданный пользователем.
Рейтинг в этом скрипте показывает, наскаолько два фильма похожт друг на друга по заданным критериям,
где каждый критерий имеет свою важность.
После от туда удаляется фильм-образец, чтобы он не попадался в рекомаендации.
...
## Описание
Запрашивает у пользователя API-ключ и выводит бюджет фильма с ID 215.
...
## Описание
Ищет фильмы по части названия в уже загруженной базе данных. Выводит все названия, которые содержат введённое ключевое слово (без учёта регистра).
