...
## Описание
Выполняет проверку доступности и работоспособности API TMDB. Это «приветственный» запрос к сервису, чтобы удостовериться, что подключение настроено корректно.

## Что делает скрипт?
Скрипт отправляет простой запрос к [TMDB](https://www.themoviedb.org/) API. Если API возвращает корректный ответ — соединение установлено успешно.
...
# Файл make_own_db.py
## Описание
Автоматически собирает информацию о фильмах из анлайн-базы данных [TMDB](https://www.themoviedb.org/) и сохраняет ее в локальный JSON-файл.
Предназначен для создания собственной коллекции данных о фильмах, которую в дальнейшем можно использовать для поиска или анализа без постоянного обращения к интернету.

## Что делает скрипт?
Запрашивает у пользователя API-ключ для доступа к [TMDB](https://www.themoviedb.org/).
Обращается к сервису [TMDB](https://www.themoviedb.org/) и последовательно загружает данные о 1000 фильмах.
Сохраняет всю полученную информацию в файл MyFilmDB.json в той же папке, где находится сам скрипт.

## Как его запустить?
1. Убедитесь, что у вас установлен Python
2. Сохраните скрипт как make_own_db.py
3. Запустите через терминал:
```
...
- Библиотека urllib
- API-ключ TMDB
- Вспомогательный файл tmdb_helpers.py
